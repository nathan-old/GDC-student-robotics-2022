        ...
        for neighbor in current.neighbors:
            ...
            if temp_g_score < g_score[neighbor]:
                ...
                f_score[neighbor] = temp_g_score + \
                    find_change(neighbor.get_pos(), end.get_pos())
...
def make_grid(size):
    ...
    # What is size param, what should it be for the arena
    grid = []
    for i in range(size):
        grid.append([])
        for j in range(size):
...
def get_route_to_target(start, cans, obstacles, target):
    ...
    print(route.routes[0])
    print(str(len(route.routes[0])) + ' steps')
...
class Instructions():
    ...
    def new_instruction(self):
        ...
        self.instruction_list.append(
            [self.direction, self.distance*self.grid_distance])
    ...
    def convert(self):
        for i in range(len(self.instruction_list)):
            if self.instruction_list[i][0] == 'Reverse':
                self.instruction_list[i] = ['turn', 180], [
                    'forwards', self.instruction_list[i][1]], ['turn', -180]
            elif self.instruction_list[i][0] == 'Up':
                self.instruction_list[i:i+1] = ['turn', -90], ['forwards',
                                                               self.instruction_list[i][1]], ['turn', 90]
...
def PathFind(position, can_locations, obstacles):
    ...
    for i in range(len(can_locations)):
        for x in range(len(can_locations[i])):
            can_locations[i][x] = convert_to_grid().convert(
                can_locations[i][x])
