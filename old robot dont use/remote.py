...
class RemoteController(RobotModule):
    ...
    def __init__(self, robot, movementManager, ruggiduino_id, serial_port,
                 serial_baud_rate, shutdown_callback, wait_between_reads=0):
        self.robot = robot
        ...
        self.serial = None
        self.ruggiduino_device = self.robot.ignored_ruggeduinos[
            self.ruggiduino_id]
    ...
    def start_listening(self):
        ...
        self.running = True
        self.thread_handle = threading.Thread(target=self.run_thread).start()
    ...
    def run_thread(self):
        ...
        while self.running:
            ...
            try:
                ...
                print("Error reading from serial: {}".format(e))
                continue
            arr = data.split(',')
            if len(arr) < 6 :
                print("Invalid array length: {}, array: {}, data: {}".format(
                    len(arr), arr, data))
                continue
            ...
            for a in arr:
                if a == '' or a == ' ':
                    print("Invalid blank value: {}, pos: {}".format(a, pos))
                    skip = True
                    break
                pos += 1
            if skip:
            ...
            A_speed = max(min((round(numpy.dot(x, y)[0][0], 1)), 1), -1)
            B_speed = max(min((round(numpy.dot(x, y)[1][0], 1)), 1), -1)
            C_speed = max(min((round(numpy.dot(x, y)[2][0], 1)), 1), -1)
            #print("A: {}, B: {}, C: {}".format(A_speed, B_speed, C_speed))

            # apply this to the motors
            # TODO: move to using the move, turn angle, etc commands instead of the raw speed
            self.movementManager.set_power(A_speed, 0)
            self.movementManager.set_power(B_speed, 1)
            self.movementManager.set_power(C_speed, 2)
