...
class Position():
	def __init__(self, R):
		self.R = R
		self.movement = MovementMaster(self.R)
		...
		self.data_array = []
		self.turn_angle = 10
		print('[INITI] Position finder initialised')
	...
	def get_pos(self):
		...
		if len(seen_markers) > 1:
			...
			for i in seen_markers:
				...
				circles.append([self.marker_list[i[3]][1],self.marker_list[i[3]][2],radius])
			valid_points = [[],[]]
			for i in range(len(circles)):
				if i + 1 != len(circles):
					...
					a = 0
				points = self.get_intersections(circles[i][0],circles[i][1],circles[i][2], circles[a][0],circles[a][1],circles[a][2])
				for i in points:
			...
			if len(valid_points[0]) > 1:
				...
				bearing_array = []
				for i in seen_markers:
					bearing = math.atan2(self.marker_list[i[3]][1]-avg_x,self.marker_list[i[3]][2]-avg_y)-math.radians(i[1])
