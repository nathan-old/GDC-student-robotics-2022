...
from primatives import Vector2D, Line
try:
  import pygame
except ImportError:
  pygame = None

__all__ = ['Collision', 'Rectangle', 'World']
...
class Rectangle:
  ...
  def from_pos_size(cls, pos, size):
    return Rectangle(pos.x, pos.y, size[0], size[1])
  ...
  def as_lines(self):
      return [
        Line(Vector2D(self.x, self.y), Vector2D(self.x + self.w, self.y)),
        Line(Vector2D(self.x + self.w, self.y), Vector2D(self.x + self.w, self.y + self.h)),
        Line(Vector2D(self.x + self.w, self.y + self.h), Vector2D(self.x, self.y + self.h)),
        Line(Vector2D(self.x, self.y + self.h), Vector2D(self.x, self.y)),
        Line(Vector2D(self.x, self.y), Vector2D(self.x + self.w, self.y + self.h)),
      ]
...
def clip_line(rect, ox, oy, idx, idy):
  ...
  if mintx > minty:
    t = mintx
    normal = Vector2D(normx, 0)
  else:
    t = minty
    normal = Vector2D(0, normy)
...
def slide_response(world, origin, cols, target, filter):
  ...
  if col.normal.x != 0:
    new_target = Vector2D(new_rect.x, target.y)
  elif col.normal.y != 0:
    new_target = Vector2D(target.x, new_rect.y)
  else:
...
def bounce_response(world, origin, cols, target, filter):
  ...
  new_rect = Rectangle.from_pos_size(col.touch, origin.size)
  if col.normal.x != 0:
    new_target = Vector2D(2 * col.touch.x - target.x, target.y)
  elif col.normal.y != 0:
    new_target = Vector2D(target.x, 2 * col.touch.y - target.y)
  else:
    raise AssertionError('Normal must be non-zero')
  return new_target, world.sweep(new_rect, new_target, filter)
...
def return_response(world, origin, cols, target, filter):
  col = cols[0]
  new_rect = Rectangle.from_pos_size(col.touch, origin.size)
  new_target = Vector2D(2 * col.touch.x - target.x, 2 * col.touch.y - target.y)
  return new_target, world.sweep(new_rect, new_target, filter)
  ...
  def sweep(self, origin, target, filter):
    ...
    for other in self.rects:
      ...
      if col:
        t, n = col
        t -= 1e-10
        touch = Vector2D(origin.x + dirx * t, origin.y + diry * t)
